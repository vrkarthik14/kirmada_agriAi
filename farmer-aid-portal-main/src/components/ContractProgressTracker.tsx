...
export const ContractProgressTracker: React.FC<ContractProgressTrackerProps> = ({
  ...
  // Track when currentStage changes and force re-render
  useEffect(() => {
    setRenderTimestamp(Date.now()); // Force re-render
  }, [currentStage]);
  ...
  // Use actual calculation, not forced values
  const isCompleted = contractStatus === "completed";
  const progressPercentage = isCompleted ? 100 : currentIndex >= 0 ? (currentIndex / (stages.length - 1)) * 100 : 0;
  ...
  return (
    <div className="w-full relative">
      ...
      <div className="relative">
        {/* Progress Line */}
        <div className="absolute top-3 left-2 right-2 h-2 bg-gray-300 rounded-full border-2 border-gray-400">
          <div 
            className="h-full bg-gradient-to-r from-green-400 to-green-600 rounded-full transition-all duration-1000 ease-in-out border border-green-500"
            style={{ 
              width: `${progressPercentage}%`,
              minWidth: progressPercentage > 0 ? '12px' : '0px'
            }}
          />
        </div>
        ...
        <div className="relative flex justify-between gap-1 px-1">
          {stages.map((stage, index) => {
            ...
            return (
              <div
                key={stage.id}
                className={`flex flex-col items-center group ${isEditable ? 'cursor-pointer' : 'cursor-help'} min-w-0 flex-1 relative`}
              ...
              >
                ...
                <div
                  className={`
                    ...
                    ${isEditable && !completed && !current ? 'hover:bg-gray-300 hover:border-gray-400' : ''}
                  `}
                >
                  {completed ? (
