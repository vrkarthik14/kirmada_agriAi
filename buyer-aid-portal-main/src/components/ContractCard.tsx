...
export const ContractCard = (contract: ContractProps) => {
  ...
  // Use same default as farmer portal for consistency
  ...
  // Load progress from API when component mounts (to see farmer's updates)
  useEffect(() => {
    const loadProgressFromAPI = async () => {
      try {
        // Use farmer backend (port 8001) for contract progress sync
        ...
        console.error("API Error connecting to farmer backend:", error);
  ...
  // Add polling to check for farmer updates every 3 seconds
  useEffect(() => {
    const pollForUpdates = setInterval(async () => {
      try {
        ...
        if (result.success && result.progress) {
          ...
          // Only update if the farmer made the change and it's different
          if (progressData.updatedBy === "farmer" && progressData.currentStage !== currentStage) {
            setCurrentStage(progressData.currentStage);
            setTempStage(progressData.currentStage);
          }
        }
      } catch (error) {
        console.error("Polling error connecting to farmer backend:", error);
      }
    }, 3000);
  ...
  const handleStageUpdate = async (newStage: string) => {
    try {
      ...
      console.log("Updating contract stage to:", newStage);
      
      // TODO: Add API call to update contract stage
  ...
  const generateDummyData = () => {
    return {
      ...
      qualitySpecifications: `Grade A ${contract.cropType || "Wheat"} - Premium quality with moisture content below 12%`
    };
  };

  const downloadContract = (language: string) => {
    const data = generateDummyData();
    const userType = "buyer"; // Since this is buyer portal
  ...
  const downloadTextAsFile = (content: string, filename: string) => {
    // This function is now deprecated in favor of PDF generation
          ...
          {(contractStatus === "active" || contractStatus === "completed") && (
            <div className="pt-4">
              <ContractProgressTracker 
                ...
                isEditable={false} // Read-only for buyers - they can view but farmers control progress
              />
            </div>
          )}
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs text-gray-500 bg-blue-50 px-2 py-1 rounded">
                  📋 Read-only view (Farmer controls progress)
                </span>
              </div>
              ...
              <ContractProgressTracker 
                ...
                currentStage={currentStage} 
                contractStatus={contractStatus}
                isEditable={false} // Read-only for buyers
              />
            </div>
            ...
            <div className="space-y-3">
              <div className="flex gap-2">
                <Button onClick={() => downloadContract("english")} className="w-full">
