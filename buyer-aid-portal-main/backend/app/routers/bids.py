...
async def get_all_bids(
    ...
    try:
        ...
        query = bids_ref
        if campaign_id:
            query = query.where('campaignId', '==', campaign_id)
        if bidder_type:
            query = query.where('bidderType', '==', bidder_type)
        if status:
...
async def get_campaign_with_bids(campaign_id: str):
    ...
    try:
        ...
        bids_ref = db.collection('bids').where('campaignId', '==', campaign_id)
...
async def create_bid(bid: BidCreate):
    ...
    try:
        ...
        # Update campaign with bid information and status
        campaign_doc = db.collection('campaigns').document(bid.campaign_id).get()
        if campaign_doc.exists:
            campaign_data = campaign_doc.to_dict()
            total_bids = campaign_data.get('totalBids', 0) + 1
            
            # Determine new status based on bid type
            new_status = "active"
            if bid.bidder_type == "farmer":
                # When farmer bids, campaign should show "Awaiting buyer response"
                current_bid_display = f"{bid.bid_amount} (Awaiting buyer response)"
            else:
                # When buyer counter-offers, show the amount
                current_bid_display = bid.bid_amount
            
            campaign_updates = {
                'totalBids': total_bids,
                'currentBid': current_bid_display,
...
async def _create_contract_from_bid(db, bid_data, bid_id):
    ...
    try:
        ...
        existing_contracts = list(db.collection('contracts').where('originalCampaignId', '==', campaign_id).stream())
        if existing_contracts:
            print(f"⚠️ Contract already exists for campaign {campaign_id}, skipping creation")
            return
        ...
        # Generate contract ID and save to buyer backend
        contract_id = str(uuid.uuid4())
        contract_data['id'] = contract_id
        db.collection('contracts').document(contract_id).set(contract_data)
        print(f"✅ Contract created successfully in buyer backend: {contract_id}")
        
        # Sync contract to farmer backend
        await _sync_contract_to_farmer_backend(contract_data)
...
async def _sync_contract_to_farmer_backend(contract_data):
    """Helper function to sync contract to farmer backend"""
    try:
        import requests
        print(f"🔄 Attempting to sync contract to farmer backend: {contract_data.get('id')}")
        
        # Create contract in farmer backend using requests (synchronous)
        response = requests.post(
            "http://localhost:8001/api/contracts/",
            json=contract_data,
        ...
        if response.status_code == 201:
            print(f"✅ Contract synced successfully to farmer backend: {contract_data.get('id')}")
        else:
            print(f"❌ Failed to sync contract to farmer backend: {response.status_code} - {response.text}")
                    
    except Exception as e:
        print(f"❌ Error syncing contract to farmer backend: {str(e)}")
        import traceback
        traceback.print_exc()
